...
export default function LandingPage() {
  ...
  // Auto-authenticate from server env if available, then redirect
  useEffect(() => {
    ...
    // Check if the server has a pre-configured API key
    fetch("/api/auto-auth")
      .then((res) => res.json())
      .then(async (data) => {
        if (data.apiKey) {
          ...
          if (validateRes.ok && validateData.data) {
            useAuthStore.getState().setApiKey(data.apiKey);
            useAuthStore.getState().setUsageStats(validateData.data);
            router.push("/dashboard");
          }
        }
      })
